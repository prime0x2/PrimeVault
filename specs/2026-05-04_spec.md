...
> **This document is a point-in-time design spec.** The numbered sections
> below describe the intended v1 design as of 2026-05-04. Where reality
> has diverged since (data model tweaks, UI rebrand, etc.), see
> [§0 Amendments](#0-amendments-since-v1) immediately below — it is the
> authoritative pointer to what changed and where the truth now lives.

---

## 0. Amendments since v1

The shape of the project has shifted since the spec was written.
Cryptographic design (§3, §4), threat model (§3), storage layout (§6),
messaging protocol (§7 / §13), permissions (§13.1), and CSP (§13.2) are
all unchanged. The drift is in the data model and the UI layer.

### Data model (§5)

- **`Entry.scope`** added — optional `'personal' | 'work'`. Surfaced as a
  segmented control in the add/edit form and as a meta-line tag in the
  list row. Purely additive — old entries without `scope` continue to
  validate. No `schemaVersion` bump.
- **`Prefs.accent`** removed. The original "Slate / Violet / Green"
  accent picker was never built; the theme system in its place
  (Direction B for dark, Direction C for light) drives the accent off
  the theme itself.
- **`Prefs.shortcutHint`** removed. The empty-state shortcut hint was
  never wired; the pref persisted but nothing read it.
- **`Prefs.clipboardClearSeconds`** options changed from
  `0 | 15 | 30 | 60` to `0 | 30 | 60`. Reason: Chrome's `chrome.alarms`
  floors any non-dev alarm delay to ~30 seconds in production, so the
  15s option silently behaved as 30s. Removing it means the user-facing
  delay always matches what the alarm actually does.

The live source of truth is `src/storage/schema.ts` (entries) and
`src/storage/prefs.ts` (preferences). If you're reading the spec and
something doesn't match what's in those files, the files win.

### UI / theme (§10)

- The original §10.1 theme model — system-aware palette plus a 3-swatch
  accent picker (Slate / Violet / Green) — was replaced with a
  **two-direction theme system**:
  - **Direction B (Terminal)** renders in dark mode: phosphor green on a
    warm-black surface, Geist Mono command-line aesthetic
    (`›` eyebrows, `$` input prefixes, `terminal-only` flourishes).
  - **Direction C (Calm)** renders in light mode: ember orange on
    near-white, Geist Sans only — `--font-mono` aliases to
    `--font-sans` so any `font-mono` utility in shared components
    automatically falls back to sans.
  - System preference is the default; explicit Light / Dark / System
    pickers live on the options page.
- **Filter chips** (kind + expiry urgency) on the vault list — not in
  the original §10.2 layout. Sort by entry count, fold past a cap into
  a `+N` overflow group, with the active filter pinned visible.
- **Add / edit is full-screen**, not the inline collapsing form drawn
  in §10.2.
- **Reveal** (§10.3) was dropped. Masked values are often longer than
  the row width; the inline copy is the real action, and details live
  in the row's expand panel and on the dedicated Edit screen.

### Repository layout (§14)

- Component primitives live in `src/components/terminal.tsx`
  (popup atoms) and `src/components/form.tsx` (options primitives,
  including the shared `<Alert>` for danger / warn / success). The
  `components/ui/` directory the spec drew was never built.
- Session is split across `src/messaging/session.ts` (lifecycle),
  `session-core.ts` (key holder + helpers), `session-entries.ts`
  (CRUD), `session-backup.ts` (export/import).
- Filter helpers extracted to `src/features/vault/filters.ts` with a
  17-test unit suite in `tests/features/filters.test.ts`.
- Theme tokens live in `src/styles/theme.css` (the spec's
  `styles/globals.css` placeholder).

### Tests (§11)

- The path `storage/envelope.ts` referenced in §11 should read
  `storage/vault.ts` — `envelope.ts` itself lives in `crypto/` (the
  format wrapper); `vault.ts` owns the atomic write+swap protocol
  that §11 was actually describing.
- The repo currently has 275 unit tests across 28 files (vs. the
  ~20 the spec implied).
...
**Implementation notes that affect the threat model:**

- **Service worker eviction is a fail-safe lock.** Chrome can tear down the MV3 service worker at any time. When that happens the in-memory key is lost, the auto-lock alarm is gone, and the next message starts a fresh session that looks locked from the popup's POV. This is a UX surprise (the user may see a sudden unlock prompt) but security-wise it's strictly safer than holding the key longer than configured. The session module documents this in its header comment.
- **Strength-meter check is client-side.** The minimum-score / minimum-length check in `validateSetup` happens entirely in the popup. A determined user could bypass it by editing popup state. We accept this: the threat model defends the user against external attackers, not against themselves. The SW does no second strength check on `setupVault` because the cost (an additional zxcvbn-ts load on the SW side) does not buy us defense against any threat we care about.
